...
export default async function handler(req, res) {
  ...
  try {
    const action = req.query.action || req.body?.action || 'propose-directions'

    if (action === 'propose-directions') {
      return handleProposeDirections(req.body, res)
    }

    return res.status(400).json({ error: 'Invalid action. Use: propose-directions' })
  } catch (err) {
    res.status(500).json({ error: err.message })
  }
}

async function handleProposeDirections(body, res) {
  const { projectId, companyBrief, competitorAnalysis } = body
